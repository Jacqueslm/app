  ...
  for (let i = 0; i < exec.length; i++) {
    ...
    if (open) {
      ...
      if (L ? c.l <= open.stop : c.h >= open.stop) {
        open.R += open.t1 ? 0 : -1;
        trades.push({R: open.R, how: open.t1 ? 'BE' : 'stop', t: open.tIn, feat: open.feat, risk: open.risk, dir: open.dir}); open = null;
      } else {
        const hitT1 = half && !open.t1 && (L ? c.h >= open.T1 : c.l <= open.T1);
        const hitT2 = L ? c.h >= open.T2 : c.l <= open.T2;
        if (hitT1 && hitT2) { open.R += half + (1 - half) * open.room; trades.push({R: open.R, how: 'T2', t: open.tIn, feat: open.feat, risk: open.risk, dir: open.dir}); open = null; }
        else {
          if (hitT1) { open.t1 = true; open.R += half; open.stop = open.entry; }
          if (open && hitT2 && (open.t1 || !half)) { open.R += (open.t1 ? 1 - half : 1) * open.room; trades.push({R: open.R, how: 'T2', t: open.tIn, feat: open.feat, risk: open.risk, dir: open.dir}); open = null; }
        }
      }
      if (open && et.hm >= sessTo) {
        const px = c.c, r = (open.dir === 1 ? px - open.entry : open.entry - px) / open.risk;
        open.R += (open.t1 ? (useT1 ? 0.5 : 1) : 1) * r;
        trades.push({R: open.R, how: 'EOD', t: open.tIn, feat: open.feat, risk: open.risk, dir: open.dir}); open = null;
      }
    }
        ...
        if (up ? c.l < a.HL : c.h > a.HL) {
          ...
          if (entryMode === 'anticipate' && !open && took < maxPerDay &&
            ...
            if (risk > 0 && (up ? stop < entry : stop > entry) && room >= minRoom) {
              ...
              if (up ? c.l <= stop : c.h >= stop) { open.R = -1; trades.push({R: -1, how: 'stop', t: c.t, feat: open.feat, risk: open.risk, dir: open.dir}); open = null; }
            }
          }
        }
        return;
        ...
        if (up ? c.l <= s.LH : c.h >= s.LH) {
          if (!open && took < maxPerDay && et.hm >= sessFrom && et.hm <= sessTo) {
            ...
            if (open && (up ? c.l <= open.stop : c.h >= open.stop)) {
              trades.push({R: -1, how: 'stop', t: c.t, feat: open.feat, risk: open.risk, dir: open.dir});
              open = null;
            }
          }
        }
