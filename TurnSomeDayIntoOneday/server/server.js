...
// Full content for the paid lesson days (16-30). Lives here under server/ (not web-served)
// so it can only be reached through the Pro-entitlement check on GET /api/lessons/pro.
// Generated by data/build-lessons.js alongside the public data/lessons.json.
let PRO_LESSONS = {};
try {
  PRO_LESSONS = require('./lessons-pro.json');
} catch (e) {
  console.warn('lessons-pro.json not found - /api/lessons/pro will 503. Run: node data/build-lessons.js');
}
const {
  COOKIE_NAME,
  hashPassword,
...
// Full content for the paid lesson days, served only to a server-verified Pro user. This is
// the real paywall: the client can flip its local isPro flag, but the paid lesson TEXT never
// leaves the server without an active subscription confirmed here.
app.get('/api/lessons/pro', requireAuth, (req, res) => {
  if (!Object.keys(PRO_LESSONS).length) {
    return res.status(503).json({ error: 'Pro lessons are not available right now.' });
  }
  const user = db.getUserById(req.userId);
  if (!user || !billing.getBillingStatus(user).isPro) {
    return res.status(403).json({ error: 'Pro subscription required.' });
  }
  res.json(PRO_LESSONS);
});
